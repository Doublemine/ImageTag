...
@SuppressWarnings("unused") public class TagLayout extends FrameLayout
  ...
  private final static double ARROW_DIRECTION_PERCENT = 0.8;
  ...
  private void addItem(int x, int y) {
    ...
    /*
    //如果需要待所有TagView都固定才能添加新的TagView，则可以去除此处注释。
    if (!checkTagsLocked()) {
      return;
    }
    */
    ...
    if (y < getHeight() * ARROW_DIRECTION_PERCENT) {
  ...
  /**
   * @return 检查是否所有的TagView状态都为Lock，以便于添加新的TagVie。全部Lock返回True否则返回False
   */
  private boolean checkTagsLocked() {
    boolean locked = true;
    for (int index = 0; index < getChildCount(); index++) {
      View view = getChildAt(index);
      if (view instanceof TagView && ((TagView) view).getStatus() == TagView.Status.Normal) {
        locked = false;
        break;
      }
    }
